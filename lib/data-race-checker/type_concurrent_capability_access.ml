...
(* There is another capability in the struct that can both access subordinate state in one
   ...
   for them. *)
   let capabilities_have_subord_channel struct_name struct_defns capability_1_name
   capability_2_name =
 let get_cap_subord_fields capability_name =
   List.filter
     ~f:(fun (TField (_, field_type, _, _)) ->
       type_has_mode field_type Subordinate struct_defns)
     (get_struct_capability_fields struct_name capability_name struct_defns) in
...
(* Check that overlapping fields are safe - i.e. either we're accessing them with a safe
 ...
 let capability_1_fields =
   get_struct_capability_fields struct_name capability_1_name env in
 let capability_2_fields =
   get_struct_capability_fields struct_name capability_2_name env in
         ...
         Result.all_unit
           (List.map
              ~f:(fun (TCapability (_, capability_thread2_name) as capability_thread2) ->
                if
                  can_concurrently_access_capabilities obj_struct struct_defns
...
let rec type_concurrent_capabilities_constraints_var struct_defns obj_name obj_struct
      ...
      Result.all_unit
        (List.map
          ~f:(fun thread_2_capabilities ->
            type_concurrent_capability_pair_constraints_var struct_defns obj_struct
              obj_name thread_1_capabilities thread_2_capabilities loc)
          other_threads_capabilities)
      >>= fun () ->
      type_concurrent_capabilities_constraints_var struct_defns obj_name obj_struct
        other_threads_capabilities loc

let type_concurrent_capability_constraints_vars struct_defns threads_free_vars loc =
  let var_names_and_structes =
    List.dedup_and_sort
      ~compare:(fun a b -> if phys_equal a b then 0 else 1)
      (List.map
          ~f:(fun (var_name, struct_name, _) -> (var_name, struct_name))
          threads_free_vars) in
  Result.all_unit
    (List.map (* check constraint for each object *)
        ~f:(fun (obj_name, obj_struct) ->
          List.filter_map
            ~f:(fun (var_name, struct_name, capabilities) ->
              if A.Var_name.(=) var_name obj_name && A.Struct_name.(=) struct_name obj_struct then Some capabilities
              else None)
            threads_free_vars
          |> fun all_threads_obj_capabilities ->
          type_concurrent_capabilities_constraints_var struct_defns obj_name obj_struct
            all_threads_obj_capabilities loc)
        var_names_and_structes)