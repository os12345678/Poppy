...
function_defns env (block_expr: block_expr) obj_vars_and_capabilities ~ignore_data_races : (block_expr Or_error.t) =
  ...
  |> type_subord_capabilities_block_expr struct_defns obj_vars_and_capabilities
  |> type_async_capabilities_block_expr struct_defns 
  |> type_linear_capabilities_block_expr struct_defns 
  >>= fun typed_block_expr ->
    Result.ignore_m (type_consume_block_expr struct_defns typed_block_expr [])
  >>= fun () ->
    type_function_forward_borrowing_block_expr method_defns function_defns struct_defns typed_block_expr
  >>= fun () ->
  Result.ignore_m (type_assign_borrowed_block_expr method_defns function_defns struct_defns typed_block_expr)
  >>= fun () ->
