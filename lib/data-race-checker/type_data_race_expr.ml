...
let type_data_races_block_expr env (block_expr: block_expr) _obj_vars_and_capabilities =
  let open Result in
  let typed_block_expr = type_read_capabilities_block_expr block_expr in
  type_linear_capabilities_block_expr env typed_block_expr
  >>= fun typed_block_expr ->
  let typed_block_expr = type_async_capabilities_block_expr env typed_block_expr in
  type_function_forward_borrowing_block_expr env typed_block_expr
  >>= fun () ->
  Result.ignore_m (type_assign_borrowed_block_expr env typed_block_expr)
  >>= fun () ->
  Ok typed_block_expr
