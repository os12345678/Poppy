...
let type_data_races_method_defn struct_name trait_name env struct_defns trait_defns impl_defns method_defns function_defns ~ignore_data_races
    (TMethod (method_sig, body_expr)) =
  let open Result in
  let param_obj_var_capabilities =
    params_to_obj_vars_and_capabilities env method_sig.params in
  ...
  type_function_reverse_borrowing env error_prefix method_sig.return_type method_sig.borrowed
    body_expr
  >>= fun () ->
  type_subord_capabilities_method_prototype env struct_name method_sig.name method_sig.return_type
