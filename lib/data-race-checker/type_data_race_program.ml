...
(Prog (struct_defns, trait_defns, impl_defns, function_defns, main_expr)) ~ignore_data_races = 
  let open Result in
    Result.all (List.map ~f:(fun (TImpl (trait_name, struct_name, methods)) ->
      ...
      >>| fun checked_methods -> TImpl (trait_name, struct_name, checked_methods))
    impl_defns) 
    ...
    >>= fun data_race_checked_function_defns ->
      ...
      >>| fun data_race_checked_main_expr ->
        (Prog (struct_defns, trait_defns, data_race_checked_impl_defns, data_race_checked_function_defns, data_race_checked_main_expr))