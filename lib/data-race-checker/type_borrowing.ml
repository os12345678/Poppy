...
let check_arg_borrowing env loc ((Param (param_type, _, _, maybe_borrowed) as param), arg_expr) = 
  ...
  let rec type_function_forward_borrowing_expr env expr =
    ...
    | TMethodApp (obj_name, obj_struct, _trait_name, meth_name, args) ->
      ...
      >>= fun () ->
      (match get_method_params meth_name env with
       | Ok params ->
          Result.all_unit
          ...
          Result.all_unit
            (List.map
               ~f:(type_function_forward_borrowing_expr env)
               args)
       | Error e -> Error e)  (* Handle error appropriately *)

    | TFunctionApp (func_name, args) ->
    let args_ids = List.concat_map ~f:reduce_expr_to_obj_ids args in
    Type_linear_capabilities.type_linear_args env args_ids expr.loc
    >>= fun () ->
      (match get_function_params func_name env with
        | Ok params ->
          Result.all_unit
            (List.map ~f:(check_arg_borrowing env expr.loc) (List.zip_exn params args))
              >>= fun () ->
    ...
    | T.TBlockExpr block_expr ->
        (type_function_forward_borrowing_block_expr env) block_expr
    | T.TConstructor (_, _, constructor_args) ->
        Result.all_unit
          (List.map
              ~f:(fun (ConstructorArg (_, expr)) ->
                type_function_forward_borrowing_expr env expr)
              constructor_args)
    | T.TLet (_, _, bound_expr) ->
        type_function_forward_borrowing_expr env bound_expr
    | T.TAssign (_, assigned_expr) ->
        type_function_forward_borrowing_expr env assigned_expr
    | T.TConsume _ -> Ok ()
    | T.TPrintf (_, args) ->
        Result.all_unit
          (List.map
        ...
        Result.all_unit
          (List.map
              ~f:(fun (AsyncExpr (_, expr)) ->
                (type_function_forward_borrowing_block_expr env) expr)
              async_exprs)
        >>= fun () ->
        (type_function_forward_borrowing_block_expr env)
          curr_thread_expr
    | T.TIf (cond_expr, then_expr, else_expr) ->
        type_function_forward_borrowing_expr env cond_expr
        >>= fun () ->
        (type_function_forward_borrowing_block_expr env) then_expr
        >>= fun () ->
        (type_function_forward_borrowing_block_expr env) else_expr
    | T.TWhile (cond_expr, loop_expr) ->
        type_function_forward_borrowing_expr env cond_expr
        >>= fun () ->
        (type_function_forward_borrowing_block_expr env) loop_expr
    | T.TBinOp (_, expr1, expr2) ->
        type_function_forward_borrowing_expr env expr1
        >>= fun () -> type_function_forward_borrowing_expr env expr2
    | T.TUnOp (_, expr) ->
        type_function_forward_borrowing_expr env expr
  
  and type_function_forward_borrowing_block_expr env (Block (_, _, exprs)) =
    Result.all_unit
      (List.map ~f:(type_function_forward_borrowing_expr env) exprs)
...
let rec type_assign_borrowed_expr env expr =
  let open Result in
  match expr.node with
  | T.TAssign (_, assigned_expr) -> (
      type_assign_borrowed_expr env assigned_expr
      >>= function
      | Some Borrowed ->
          Error
  ...
  | T.TMethodApp ( _, _, _, meth_name, args) ->
    ...
    >>| fun _ ->
    get_method_defn meth_name env
    |> fun (Poppy_parser.Ast.TMethod (method_signature, _)) -> method_signature.borrowed

  | T.TFunctionApp (func_name, args) ->
    (* Recurse on arguments *)
    Result.all (List.map ~f:(type_assign_borrowed_expr env) args)
    >>| fun _ ->
    get_function_defn func_name env
    |> fun (Poppy_parser.Ast.TFunction (function_signature, _)) -> function_signature.borrowed

  | T.TInt _ | T.TBoolean _ -> Ok None
  | T.TIdentifier id -> Ok (id_maybe_borrowed id)
  | T.TBlockExpr block_expr ->
      (type_assign_borrowed_block_expr env) block_expr
  | T.TConstructor (_, _, constructor_args) ->
      Result.all
        (List.map
            ~f:(fun (ConstructorArg (_, expr)) ->
              type_assign_borrowed_expr env expr)
            constructor_args)
      >>| fun _ -> None
  | T.TLet (_, _, bound_expr) ->
      type_assign_borrowed_expr env bound_expr
  ...
  | T.TFinishAsync (async_exprs, _, curr_thread_expr) ->
      Result.all
        (List.map
            ~f:(fun (AsyncExpr (_, expr)) ->
              (type_assign_borrowed_block_expr env) expr)
            async_exprs)
      >>= fun _ ->
      (type_assign_borrowed_block_expr env) curr_thread_expr
  | T.TIf (cond_expr, then_expr, else_expr) -> (
    type_assign_borrowed_expr env cond_expr
    >>= fun _ ->
    (type_assign_borrowed_block_expr env) then_expr
    >>= fun then_maybe_borrowed ->
    (type_assign_borrowed_block_expr env) else_expr
  ...
  | T.TWhile (cond_expr, loop_expr) ->
      type_assign_borrowed_expr env cond_expr
      >>= fun _ ->
      (type_assign_borrowed_block_expr env) loop_expr
      >>| fun _ -> None
  (* loop returns void *)
  | T.TBinOp ( _, expr1, expr2) ->
      type_assign_borrowed_expr env expr1
      >>= fun _ ->
      type_assign_borrowed_expr env expr2 >>| fun _ -> None
  (* we don't return borrowed ref as we don't return object *)
  | T.TUnOp (_, expr) ->
      type_assign_borrowed_expr env expr >>| fun _ -> None

  and type_assign_borrowed_block_expr env (T.Block (_, _, exprs)) =
    let open Result in
    Result.all (List.map ~f:(type_assign_borrowed_expr env) exprs)
