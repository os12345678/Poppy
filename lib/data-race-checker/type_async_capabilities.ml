...
let filter_capabilities_with_thread_or_subord_state struct_name struct_defns
    all_capabilities curr_capability =
  let thread_or_subord_capabilities =
    List.filter
      ~f:(fun capability ->
        capability_fields_have_mode capability struct_name ThreadLocal struct_defns
        || capability_fields_have_mode capability struct_name Subordinate struct_defns)
      all_capabilities in
  (* check we can concurrently access this capability with the thread or subord
     capabilities, i.e all overlapping state is safe() *)
  List.for_all
    ~f:(fun capability ->
      can_concurrently_access_capabilities struct_name struct_defns capability
        curr_capability)
    thread_or_subord_capabilities

let remove_thread_subord_caps_from_async_expr struct_defns
(AsyncExpr (free_var_types_and_capabilities, async_expr)) =
  ...
  let updated_async_expr =
    List.fold ~init:async_expr
      ~f:(fun acc_async_expr (obj_name, obj_struct, _) ->
        let obj_is_subord_of_thread =
          struct_has_mode obj_struct ThreadLocal struct_defns
          || struct_has_mode obj_struct Subordinate struct_defns in
        if obj_is_subord_of_thread then
          ...
          update_matching_identifier_caps_block_expr
            ...
            (filter_capabilities_with_thread_or_subord_state obj_struct struct_defns)
            acc_async_expr
        else acc_async_expr)
  ...
  let rec type_async_capabilities_expr struct_defns expr =
    ...
    | TFinishAsync
        ...
        let typed_async_exprs =
          List.map
            ~f:(fun async_expr ->
              remove_thread_subord_caps_from_async_expr struct_defns async_expr)
            async_exprs in
        ...
        ( List.map
            ~f:(fun (AsyncExpr (free_vars_types_and_capabilities, async_expr)) ->
              AsyncExpr
                ...
                , type_async_capabilities_block_expr struct_defns async_expr ))
            typed_async_exprs
        , curr_thread_free_vars_and_types
        , type_async_capabilities_block_expr struct_defns curr_thread_expr )
        in
  ...
  | TAssign (id, assigned_expr) -> {expr with node = 
      TAssign (id, type_async_capabilities_expr struct_defns assigned_expr)}
  | TConsume _ -> expr
  | TMethodApp (obj_name, struct_name, trait_name, method_name, obj_capabilities, args)
    -> {expr with node = 
      TMethodApp
        ...
        , method_name
        , obj_capabilities
        , List.map ~f:(type_async_capabilities_expr struct_defns) args )}
  | TFunctionApp (func_name, args) -> {expr with node = 
    TFunctionApp
      ( func_name
      , List.map ~f:(type_async_capabilities_expr struct_defns) args )}
  | TPrintf (format_str, args) -> {expr with node =
    TPrintf (format_str, List.map ~f:(type_async_capabilities_expr struct_defns) args)}
  | TIf (cond_expr, then_expr, else_expr) -> {expr with node = 
    TIf
      ( type_async_capabilities_expr struct_defns cond_expr
      , type_async_capabilities_block_expr struct_defns then_expr
      , type_async_capabilities_block_expr struct_defns else_expr )}
  | TWhile (cond_expr, loop_expr) -> {expr with node = 
      TWhile
        ( type_async_capabilities_expr struct_defns cond_expr
        , type_async_capabilities_block_expr struct_defns loop_expr )}

  | TBinOp (binop, expr1, expr2) -> {expr with node =
    TBinOp
      ( binop
      , type_async_capabilities_expr struct_defns expr1
      , type_async_capabilities_expr struct_defns expr2 )}
  | TUnOp (unop, expr) -> {expr with node = 
      TUnOp (unop, type_async_capabilities_expr struct_defns expr)}

and type_async_capabilities_block_expr struct_defns (Block (loc, type_expr, exprs)) =
  Block (loc, type_expr, List.map ~f:(type_async_capabilities_expr struct_defns) exprs)