...
let find_function_prototype function_name =
  match find_identifier !current_scope function_name with
  | Some (Variable (Ast.Id id)) -> Some id
  | _ -> None
;;
...
| Ast.ClassInstance class_name -> pointer_type (struct_type context [| |]) (* TODO: fix this *)
;;
...
let rec codegen_expr = function
  ...
  | Ast.StringLiteral s ->
    ...
    | Ast.Id s ->
      (match find_identifier current_scope s with
       | Some (Variable (Ast.Id id)) -> id
...
and codegen_statement : Ast.statement -> llvalue = function
  | Ast.FuncDecl (Ast.Id name, args, return_type, body) ->
    ...
    begin
      ...
      | None ->
        ...
        let new_scope = create_new_scope (Some !current_scope) in
        ...
        position_at_end bb builder;
        current_scope := (match !current_scope.parent with Some p -> p | None -> assert false);
  ...
  | Ast.Let ((id_decl, _), expr) ->
    ...
    ignore (build_store value alloca builder);
    ignore (add_local_variable !current_scope id (Variable (Ast.Id id)));
    const_int (i64_type context) 0
