...
let rec desugar_impl_defn (impl_defn: A.impl_defn) : F.llvm_function_defn list =
  match impl_defn with
  | A.TImpl (trait_name, struct_name, method_defns) ->
    List.map ~f:desugar_method_defn method_defns

and desugar_method_defn (method_defn: A.method_defn) : F.llvm_function_defn =
  match method_defn with
  | A.TMethod (method_signature, block_expr) ->
